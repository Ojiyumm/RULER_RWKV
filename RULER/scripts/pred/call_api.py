...
def get_llm(tokens_to_generate):
    ...
    elif args.server_type == 'rwkv7':
        from model_wrappers import RWKVModel
        llm = RWKVModel(
            name_or_path=args.model_name_or_path,
            temperature=args.temperature,
            top_k=args.top_k,
...
def get_remaining_data(task_file: str, pred_file: str) -> List[Dict[str, Any]]:
    """获取未处理的数据"""
    # 读取预测文件
    pred_index = []
    ...
    # 过滤出未预测的样本
    return [sample for sample in data if sample['index'] not in pred_index]
